...
void main() {
  ...
  test('bank account repo create/get/update/delete', () async {
    ...
    await db.rawInsert('INSERT OR IGNORE INTO account_types (id, code, name) VALUES (?, ?, ?)', ['t1', 'T1', 'Type1']);
    await db.rawInsert('INSERT OR IGNORE INTO accounts (id, name, color, icon, description, initial_balance_cents, actual_balance_cents, active, type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', ['acct-br', 'BR', '#000', 'i', null, 0, 0, 1, 't1']);

    final repo = BankAccountsRepository();
    final ba = BankAccount(accountId: 'acct-br', bankName: 'RepoBank', number: '999');
  ...
  test('credit card repo create invalid should throw', () async {
    final db = await FinTrackDb.instance.db;
    await db.rawInsert('INSERT OR IGNORE INTO account_types (id, code, name) VALUES (?, ?, ?)', ['t2', 'T2', 'Type2']);
    await db.rawInsert('INSERT OR IGNORE INTO accounts (id, name, color, icon, description, initial_balance_cents, actual_balance_cents, active, type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', ['acct-cc-r', 'CCR', '#000', 'i', null, 0, 0, 1, 't2']);

    final repo = CreditCardAccountsRepository();
    final bad = CreditCardAccount(accountId: 'acct-cc-r', bankName: 'B', lastDigits: '12A4', limitCents: 1000, closingDate: '2025-12-31', dueDate: '2026-01-15');
    try {
  ...
  test('goal, debt and loan repos create/get/update/delete', () async {
    final db = await FinTrackDb.instance.db;
    await db.rawInsert('INSERT OR IGNORE INTO account_types (id, code, name) VALUES (?, ?, ?)', ['t3', 'T3', 'Type3']);
    await db.rawInsert('INSERT OR IGNORE INTO accounts (id, name, color, icon, description, initial_balance_cents, actual_balance_cents, active, type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', ['acct-g', 'G', '#000', 'i', null, 0, 0, 1, 't3']);
    await db.rawInsert('INSERT OR IGNORE INTO accounts (id, name, color, icon, description, initial_balance_cents, actual_balance_cents, active, type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', ['acct-d', 'D', '#000', 'i', null, 0, 0, 1, 't3']);
    await db.rawInsert('INSERT OR IGNORE INTO accounts (id, name, color, icon, description, initial_balance_cents, actual_balance_cents, active, type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', ['acct-l', 'L', '#000', 'i', null, 0, 0, 1, 't3']);

    final goalRepo = GoalAccountsRepository();
    final ga = GoalAccount(accountId: 'acct-g', objective: 'Save', targetAmountCents: 10000, targetDate: '2026-01-01');
    ...
    final la = LoanAccount(accountId: 'acct-l', entity: 'Loan', amountCents: 3000);
