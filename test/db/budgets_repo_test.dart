...
void main() {
  ...
  test('createBudget, addAccountToBudget and removeAccountFromBudget', () async {
    ...
    // prepare account
    await db.rawInsert('INSERT OR IGNORE INTO account_types (id, code, name) VALUES (?, ?, ?)', ['t-b', 'TB', 'TypeB']);
    await db.rawInsert('INSERT OR IGNORE INTO accounts (id, name, color, icon, description, initial_balance_cents, actual_balance_cents, active, type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', ['acct-b2', 'AcctB2', '#000', 'i', null, 0, 0, 1, 't-b']);

    final repo = BudgetsRepository();
    final id = generateUuidV4();
    final b = Budget(id: id, entity: 'Food', period: 'monthly', targetDate: null, limitCents: 50000);
    final created = await repo.createBudget(b);
    expect(created.id, id);

    // add account
    await repo.addAccountToBudget(id, 'acct-b2');
    final rows = await db.query('budget_accounts', where: 'budget_id = ?', whereArgs: [id]);
